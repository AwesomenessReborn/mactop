...
func TestLayoutMemoryFusedRegistered(t *testing.T) {
	found := false
	for _, layout := range layoutOrder {
		if layout == LayoutMemoryFused {
			found = true
			break
		}
	}
	if !found {
		t.Fatal("LayoutMemoryFused missing from layoutOrder")
	}
	if namedLayoutSetters[LayoutMemoryFused] == nil {
		t.Fatal("LayoutMemoryFused missing from namedLayoutSetters")
	}
}
